...
export default function App() {
  ...
  const [activeZone, setActiveZone] = useState<'A' | 'B' | 'C' | 'D' | 'E'>('A'); // Zone A: Personal, Zone B: Jury, Zone C: Common, Zone D: DWWM, Zone E: Digital
  const getActiveZoneLabel = (): string => {
    switch (activeZone) {
      case 'A': return 'Zone A - Vous';
      case 'B': return 'Zone B - Jury';
      case 'C': return 'Zone C - Commun';
      case 'D': return 'Zone D - DWWM';
      case 'E': return 'Zone E - Digital CDO & SD';
      default: return '';
    }
  };
  ...
  const getFicheResources = (f: Fiche): ResourceInfo[] => {
    ...
    if (f.audio5) list.push({ key: 'audio5', type: 'audio', name: "Vocal Digital CDO & SD (.m4a)", url: f.audio5, zone: 'E' });
    if (f.slide5) list.push({ key: 'slide5', type: 'slide', name: "Slides Digital CDO & SD", url: f.slide5, zone: 'E' });
    if (f.video5) list.push({ key: 'video5', type: 'video', name: "Vidéo Digital CDO & SD", url: f.video5, zone: 'E' });
    if (f.image5) list.push({ key: 'image5', type: 'image', name: "Schéma Digital CDO & SD", url: f.image5, zone: 'E' });
    if (f.nblm5) list.push({ key: 'nblm5', type: 'nblm', name: "NotebookLM Digital CDO & SD", url: f.nblm5, zone: 'E' });
    if (f.info5) list.push({ key: 'info5', type: 'slide', name: "Document d'Information (Google Drive)", url: f.info5, zone: 'E' });
  ...
  useEffect(() => {
    const autoSyncSpreadsheet = async () => {
      ...
      try {
        const spreadsheetUrl = localStorage.getItem('m-motors-spreadsheet-url') || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQkuLkHLmUN14DNmInxhF_xdheMJg5f_0pF4lU7bcytj7-869dshb05YWSJkT5cqoD2HcUjIbU36fzm/pub?output=csv';
        const base = getBasePubUrl(spreadsheetUrl);
        const isDefaultUrl = spreadsheetUrl.includes("2PACX-1vQkuLkHLmUN14DNmInxhF_xdheMJg5f_0pF4lU7bcytj7-869dshb05YWSJkT5cqoD2HcUjIbU36fzm");
    ...
    if (window.confirm(`Voulez-vous marquer les ${matchingFiches.length} fiches sélectionnées comme "Fait" dans la Zone ${activeZone} ?`)) {
      setFiches(prev => prev.map(f => {
        ...
        if (matches) {
          ...
          } else if (activeZone === 'C') {
            return { ...f, status3: 'Fait', date3: new Date().toLocaleDateString('fr-FR') };
          } else if (activeZone === 'D') {
            return { ...f, status4: 'Fait', date4: new Date().toLocaleDateString('fr-FR') };
          } else if (activeZone === 'E') {
            return { ...f, status5: 'Fait', date5: new Date().toLocaleDateString('fr-FR') };
          }
        }
        return f;
      }));
  ...
  const fichesInZoneE = useMemo(() => fiches.filter(f => f.inZoneE === true), [fiches]);
  ...
  const stats4 = useMemo(() => {
    ...
    const fait = fichesInZoneD.filter(f => (f.status4 || 'A faire') === 'Fait').length;
    const cours = fichesInZoneD.filter(f => (f.status4 || 'A faire') === 'En cours').length;
    const faire = fichesInZoneD.filter(f => (f.status4 || 'A faire') === 'A faire').length;
    return {
      fait,
      cours,
      faire,
  ...
  const stats5 = useMemo(() => {
    ...
    const fait = fichesInZoneE.filter(f => (f.status5 || 'A faire') === 'Fait').length;
    const cours = fichesInZoneE.filter(f => (f.status5 || 'A faire') === 'En cours').length;
    const faire = fichesInZoneE.filter(f => (f.status5 || 'A faire') === 'A faire').length;
    return {
      fait,
      cours,
      faire,
  ...
  const currentStats = activeZone === 'A' ? stats1 : activeZone === 'B' ? stats2 : activeZone === 'C' ? stats3 : activeZone === 'D' ? stats4 : stats5;
  ...
  }, [activeZone, fichesInZoneA, fichesInZoneB, fichesInZoneC, fichesInZoneD, fichesInZoneE]);
            ...
            <div className="pt-3 border-t border-slate-100">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-2">
                <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                  🟢 Statut de validation ({getActiveZoneLabel()}):
                </span>
                {completedDate && (
                    ...
                    <div className="grid grid-cols-1 gap-2.5 mt-1">
                      {immersiveFiche.id >= 2000 ? (
                        <div className="flex flex-col gap-1 p-2 rounded-xl bg-slate-950/40 border border-slate-855">
                          <span className="text-[9px] font-black tracking-wider text-pink-400 uppercase">Zone E — Digital CDO & SD</span>
                          <div className="flex items-center gap-2 mt-0.5 justify-between">
                            <select
                              ...
                              className={`text-[10.5px] font-bold rounded-lg p-1 px-2 border outline-none bg-slate-900 text-slate-100 cursor-pointer ${
                                (immersiveFiche.status5 || 'A faire') === 'Fait' ? 'border-emerald-500 text-emerald-450' : (immersiveFiche.status5 || 'A faire') === 'En cours' ? 'border-yellow-500 text-yellow-450' : 'border-slate-700 text-slate-400'
                              }`}
                            >
                      ...
                      ) : immersiveFiche.id >= 1000 ? (
                        <div className="flex flex-col gap-1 p-2 rounded-xl bg-slate-950/40 border border-slate-855">
                          <span className="text-[9px] font-black tracking-wider text-violet-400 uppercase">Zone D — DWWM</span>
                          <div className="flex items-center gap-2 mt-0.5 justify-between">
                            <select
                              ...
                              className={`text-[10.5px] font-bold rounded-lg p-1 px-2 border outline-none bg-slate-900 text-slate-100 cursor-pointer ${
                                (immersiveFiche.status4 || 'A faire') === 'Fait' ? 'border-emerald-500 text-emerald-450' : (immersiveFiche.status4 || 'A faire') === 'En cours' ? 'border-yellow-500 text-yellow-450' : 'border-slate-700 text-slate-400'
                              }`}
                            >
                              <option value="A faire">⭕ A faire</option>
                              <option value="En cours">⏳ En cours</option>
                          ...
                          <div className="flex flex-col gap-1 p-2 rounded-xl bg-slate-950/40 border border-slate-855">
                            ...
                            <div className="flex items-center gap-2 mt-0.5 justify-between">
                              <select
                                ...
                                className={`text-[10.5px] font-bold rounded-lg p-1 px-2 border outline-none bg-slate-900 text-slate-100 cursor-pointer ${
                                  immersiveFiche.status2 === 'Fait' ? 'border-emerald-500 text-emerald-400' : immersiveFiche.status2 === 'En cours' ? 'border-yellow-550 text-yellow-450' : 'border-slate-700 text-slate-400'
                                }`}
                              >
                                <option value="A faire">⭕ A faire</option>
                                <option value="En cours">⏳ En cours</option>
        ...
        <div className="w-full flex justify-center mb-8 px-4">
          <div className={`p-1.5 bg-slate-900 border border-slate-800 rounded-[24px] shadow-lg flex flex-wrap sm:flex-nowrap gap-1.5 w-full max-w-4xl transition-all duration-300 ring-4 hover:scale-[1.01] ${
            ...
            <button
               onClick={() => {
                 setActiveZone('A');
                 triggerToast("Zone A activée : Ma Progression Personnelle (Moi) 🎯", "info");
               }}
            ...
            <button
               onClick={() => {
                 setActiveZone('C');
                 triggerToast("Zone C activée : Formation Python 🐍", "info");
               }}
               className={`flex-1 py-2 px-3 rounded-2xl text-[10px] md:text-xs font-bold uppercase flex items-center justify-center gap-1.5 transition-all duration-300 cursor-pointer transform active:scale-95 ${
                 activeZone === 'C'
            ...
            <button
               ...
               className={`flex-1 py-2 px-3 rounded-2xl text-[10px] md:text-xs font-bold uppercase flex items-center justify-center gap-1.5 transition-all duration-300 cursor-pointer transform active:scale-95 ${
                 activeZone === 'D'
                   ...
                   : 'text-slate-400 hover:text-slate-200 hover:bg-slate-855/50'
               }`}
            >
            ...
            <button
               onClick={() => {
                 setActiveZone('E');
                 triggerToast("Zone E activée : Digital CDO & SD 柔", "info");
               }}
               className={`flex-1 py-2 px-3 rounded-2xl text-[10px] md:text-xs font-bold uppercase flex items-center justify-center gap-1.5 transition-all duration-300 cursor-pointer transform active:scale-95 ${
                 activeZone === 'E'
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-5 gap-1">
                <button
                  onClick={() => {
                    setActiveZone('A');
                    triggerToast("🎯 Zone A (Moi) activée !", "success");
                ...
                <button
                  onClick={() => {
                    setActiveZone('B');
                    triggerToast("📢 Zone B (Jury) activée !", "success");
                    window.scrollTo({ top: 400, behavior: 'smooth' });
                  }}
                  className={`py-1 text-xs font-black rounded-lg text-center transition-all cursor-pointer ${
                ...
                <button
                  onClick={() => {
                    setActiveZone('C');
                    triggerToast("🐍 Zone C (Formation Python) activée !", "success");
                    window.scrollTo({ top: 400, behavior: 'smooth' });
                  }}
                  className={`py-1 text-xs font-black rounded-lg text-center transition-all cursor-pointer ${
                  ...
                  title="Sauter à la Zone C (Formation Python)"
                >
                  C
                </button>
                <button
                  onClick={() => {
                    setActiveZone('D');
                    triggerToast("🧭 Zone D (Formation DWWM) activée !", "success");
                    window.scrollTo({ top: 400, behavior: 'smooth' });
                  }}
                  className={`py-1 text-xs font-black rounded-lg text-center transition-all cursor-pointer ${
                ...
                <button
                  onClick={() => {
                    setActiveZone('E');
                    triggerToast("柔 Zone E (Digital CDO & SD) activée !", "success");
                    window.scrollTo({ top: 400, behavior: 'smooth' });
                  }}
                  className={`py-1 text-xs font-black rounded-lg text-center transition-all cursor-pointer ${
                    activeZone === 'E' ? 'bg-pink-600 text-white shadow-md shadow-pink-500/20 scale-105' : 'bg-slate-800 hover:bg-slate-700 text-slate-300'
                  }`}
                  title="Sauter à la Zone E (Digital CDO & SD)"
                >
                  E
                </button>
              </div>
